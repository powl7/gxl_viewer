...
def save_graph_as_gxl(nxgraph, outfile, graphname=''):
    ...
    if nxgraph.is_directed():
        ...
        edgemode="undirected"
    graph = ElementTree.SubElement(root, "graph", edgeids="false", edgemode=edgemode, id=graphname)
    ...
    for node_name, node_attr in nxgraph.items():
    ...
    for e1 in nxgraph.edge:
        for e2, edge_attr in nxgraph.edge[e1].items():
            if e2 not in edges[e1]:
                edges[e1].append((e2, edge_attr))

    # edges = {e1: [e2 for e2 in nxgraph.edge[e1] if e1 <= e2] for e1 in nxgraph.edge}
    # edges = {k: v for k, v in edges.items() if v}
    for e1 in edges:
        for e2, edge_attr in edges[e1]:
            edge = ElementTree.SubElement(graph, "edge",
                                          attrib={"from": '{}_{}'.format(graphname, e1),
                                                    "to": '{}_{}'.format(graphname, e2)})
            for attr_name, attr_value in edge_attr.items():
                a = ElementTree.SubElement(edge, "attr", name=attr_name)
                ElementTree.SubElement(a, "float").text = str(attr_value)
    ...
    for elem in graph:
        if elem.tag == "attr":
            ...
            for attr_name, attr_value in pos_dict.items():
                ...
                if attr_name in ('x', 'y'):
                    continue
                nxgraph[node_name][attr_name] = attr_value
            ...
            nxgraph.add_edge(e1, e2, **attr_dict)
